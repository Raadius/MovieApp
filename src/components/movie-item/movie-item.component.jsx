...
const MovieItem = (props) => {
  const { title, release_date, poster, description, votes, genres, raiting, id } = props.item;
  const { setMovieRating } = props;

  const [itemRating, setItemRaiting] = useState('');
  ...
  function onCLickRaiting(value) {
    setMovieRating(value);
    setItemRaiting(value);
  }

  const getStars = (rating, id) => {
    if (rating) {
      return rating;
    }
    try {
  ...
  const hasGenres = genres.length > 0 ? <GenreList genres={genres} /> : null;
